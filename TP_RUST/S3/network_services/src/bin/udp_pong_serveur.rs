    ...
    loop {
        match socket.recv_from(&mut buffer) { // Attend de recevoir des données d'un client
            Ok((size, client_addr)) => {
                ...
                let response = "PONG".as_bytes(); // Convertit la chaîne de caractères "PONG" en tableau de bytes
                match socket.send_to(response, client_addr) { // Envoie la réponse au client
                    Ok(_) => println!("Sent PONG to {}", client_addr), // Affiche un message de succès si l'envoi est réussi
                    Err(err) => eprintln!("Failed to send PONG to {}: {}", client_addr, err), // Affiche un message d'erreur si l'envoi échoue
                }
            }
